...
def test_process_raw_tweets():
	...
	#the following test casese focus on getting rid of unncessary symbols or strings 
	...
	assert process_tweets.process_raw_tweet("don't you like @Kinagrannis's music") =="don't you like music"	
	assert process_tweets.process_raw_tweet("i am just super confused right now @@, aren't you") =="i am just super confused right now aren't you" 
	assert process_tweets.process_raw_tweet("hello world @LitieDaZhu")== 'hello world '
	#testing the implementations dealing with hashtags 
	assert process_tweets.process_raw_tweet("#LOL!") == "lol,"
	assert process_tweets.process_raw_tweet("#i can graduate this semester woohoo#")=="i can graduate this semester woohoo#"
	assert process_tweets.process_raw_tweet("i think the best #csclass I've taken at #rpi is possibly #sdd") =="i think the best csclass i've taken at rpi is possibly sdd"
	assert process_tweets.process_raw_tweet("this is the best place ever www.rpi.edu") =='this is the best place ever '
	#testing the implemntaitions dealing with websites/http related
	assert process_tweets.process_raw_tweet('wanna check out this site later https://www.ohbay.com please?')=="wanna check out this site later please,"
	assert process_tweets.process_raw_tweet("what is that site again i forgot http://wwww.ratemyprofe, what's next?") =="what is that site again i forgot what's next,"
	
	#testing the remove-white-space feature 
	assert process_tweets.process_raw_tweet("heyo              boi!")=="heyo boi,"
	assert process_tweets.process_raw_tweet("i just wanna use                  to troll, white space that is!")=="i just wanna use to troll, white space that is,"
	
	#testing punctuations
	assert process_tweets.process_raw_tweet("so.....what's next ,,,???!!!!")=="so,,,,,what's next ,,,,,,,,,,"
	assert process_tweets.process_raw_tweet("why can't school be less depressing?!")=="why can't school be less depressing,,"
...
def test_extract_fetched_tweets(): 
	tweets,processed_tweets=process_tweets.extract_fetched_tweets("raw_twitter_input.json")
...
#Moreover, the sentiment analyzer should ideally generate a sentiment for each tweet (no empty string)
