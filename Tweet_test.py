...
def test_TweetClass_single_tweet():
    ...
    assert tweet_object2.__str__(
    ) in "text: Litie is looking fly. | sentiment: "

    tweet3 = "Mike is happy to be in slide-to-display."
    tweet_object3 = Tweet.Tweet(tweet3, '', 'positive')
    assert tweet_object3.__str__(
    ) in "text: Mike is happy to be in slide-to-display. | sentiment: positive"

    tweet4 = "Rachel fails to handle her stress."
    tweet_object4 = Tweet.Tweet(tweet4, '', 'negative')
    assert tweet_object4.__str__(
    ) in "text: Rachel fails to handle her stress. | sentiment: negative"
